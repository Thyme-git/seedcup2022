...
def actionResp2info(packet: PacketResp) -> torch.Tensor:
    ...
    if packet.type == PacketType.GameOver:
        return packet.data.scores[-1], torch.zeros((5,16,16)), True, False
            ...
            for obj in block.objs:
                if obj.type == ObjType.Character:
                    if obj.status.playerID == 1 and not obj.status.isAlive:
                        total_reward -= 1
                    if obj.status.playerID == playerId and obj.status.slaveWeapon.attackCDLeft == 0:
                        slavryCD = True
    ...
    return total_reward, state, done, slavryCD
    ...
    for epoch in range(config.n_epoch):
        ...
        while not connected:
            try:
                ...
                agent.save('./model_v3.pt')
                scores = resp.data.scores
                with open('./score_logs_v3', 'a') as f:
                    f.write(str(scores) + '\n')
                    f.close()
            except:
