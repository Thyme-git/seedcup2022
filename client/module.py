...
def actionResp2info(packet: PacketResp) -> torch.Tensor:
    '''将actionResp转换成reward, state, done, slavryCD'''
    if packet.type == PacketType.GameOver:
        return packet.data.scores[-1], torch.zeros((3*16*16,)), True, False
    ...
    # [0]:颜色信息， [1] 墙体信息， [2]:人物以及buff以及副武器
    state = torch.zeros((3, 16, 16))
        ...
        if len(block.objs):
            for obj in block.objs:
                if obj.type == ObjType.Character:
                    # if obj.status.playerID == 1 and not obj.status.isAlive:
                    #     total_reward -= 1
                    if obj.status.playerID == 1 and obj.status.slaveWeapon.attackCDLeft == 0:
                        slavryCD = True
                    state[2][block.x][-block.y] += obj.status.playerID + 1
                elif obj.type == ObjType.Item:
                    state[2][block.x][-block.y] += obj.status.buffType + 10    
                elif obj.type == ObjType.SlaveWeapon and obj.status.playerID != playerId:
                    state[2][block.x][-block.y] -= 20

    return total_reward, state.view(-1), done, slavryCD
...
class Qnet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3*16*16, 512)
        self.fc2 = nn.Linear(512, 128)
        self.fc_A = nn.Linear(128, 6)
...
class Agent:
    def __init__(self, config:Config):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, 10, 0.99)
...
# 撞墙扣除CD
'''return 0 撞墙， 1：正常， 2： 吃到了buff'''
def knockAtWall(action_int, state, playId = 2):
    obs = state.view((3, 16, 16))
    ...
    if next_x < 0 or next_y < 0 or next_x > 15 or next_y > 15 or obs[1][next_x][next_y] == 0.0:
        return 0
    if (next_x, next_y) in grayLocations:
        return 0
    if obs[2][next_x][next_y] >= 10:
        return 2
    return 1
    ...
    for epoch in range(config.n_epoch):
        ...
        while not connected:
            try:
                ...
                # print(state.view((3, 16, 16))[0])
                # print(state.view((3, 16, 16))[1])
                # print(state.view((3, 16, 16))[2])
                ...
                while resp.type != PacketType.GameOver:
                    ...
                    # check = knockAtWall(action_int, state)
                    # if check == 0:
                    #     next_total_reward -= 5
                    # if check == 2:
                    #     next_total_reward += 20
                ...
                agent.save('./modelv2.pt')
                scores = resp.data.scores
                with open('./score_logs', 'a') as f:
                    f.write(str(scores) + '\n')
                    f.close()
            except:
                print('conneted error')
                connected = False
