...
def Chase_filter(pos, aims, go_Vacant: bool = False):
    filter1 = aims[[i for i in range(aims.shape[0]) if Distance(aims[i], pos) > 1 + int(go_Vacant)]]
    filter2 = filter1[[i for i in range(filter1.shape[0]) if aims[i, 0] < 21 and aims[i, 1] < 21]]
    return filter2


def state2actions(state, characters):
    global count, count1, count2, count3, round_count

    obs = state[:-10].view((5, 24, 24))
    CON, PRO = characters[0], characters[1]
    pos = [torch.tensor([PRO[0].x, -PRO[0].y]) if len(PRO) else None,
           torch.tensor([PRO[1].x, -PRO[1].y]) if len(PRO) == 2 else None]
    aim_pos = [torch.tensor([CON[0].x, -CON[0].y]) if len(CON) else None,
               torch.tensor([CON[1].x, -CON[1].y]) if len(CON) == 2 else None]
    ...
    if round_count is None:
        ...
        for i in range(len(PRO)):
            ...
            if PRO[i].hp <= 50:
                ...
                elif speed_indices.shape[0] != 0 and PRO[i].moveCD >= 2:
                    speed_pos = Nearest(pos[i], speed_indices)
                    d[i] = Select_dir(pos[i], speed_pos)
                    continue

                elif vacant_indices.shape[0] != 0:
                ...
                elif hp_indices.shape[0] != 0 and PRO[i].hp < 100 and d[i] == -1:
                    hp_pos = Nearest(pos[i], hp_indices)
                    d[i] = Select_dir(pos[i], hp_pos)
                    continue
...
def main():
    ...
    with Client() as client:
        ...
        init_packet = PacketReq(PacketType.InitReq, [init_req] + [init_req])
